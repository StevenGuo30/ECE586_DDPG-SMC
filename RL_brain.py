...
class ReplayBuffer:
    ...
    def sample(self, batch_size):
        ...
        return np.array(state), np.vstack(action), np.array(reward), np.array(next_state), done
    # 测量当前时刻的队列长度
    def size(self):
...
class DDPG:
    ...
    def take_action(self, state):
        # 维度变换 list[n_states] --> tensor[1, n_states] --> gpu
        state = torch.tensor(state, dtype=torch.float).view(1, -1).to(self.device)
        # 策略网络计算出当前状态下的动作价值 [1, n_states] --> [1, n_actions]
        action = self.actor(state).detach().cpu().numpy()
        # 给动作添加噪声，增加搜索
        noise = self.sigma * np.random.randn(self.n_actions)
        # 注意：这里假设 action 和 noise 形状匹配
        action = action + noise
        return action
    ...
    def update(self, transition_dict):
        ...
        actions = torch.tensor(transition_dict['actions'], dtype=torch.float).to(self.device)  # [b,n_actions]
        # print(actions.shape)
