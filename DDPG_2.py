...
reward_list = []  # 新增：记录每个回合中的每一步回报

for i in tqdm(range(10)):  # 迭代10回合
    episode_return = 0  # 累计每条链上的reward
    episode_rewards = []  # 新增：记录当前回合的每一步回报
    ...
    while not done:
        ...
        episode_rewards.append(reward)  # 新增：记录当前步的回报
        print(reward)
    ...
    reward_list.append(episode_rewards)  # 新增：将当前回合的回报列表添加到总列表中
...
for i, rewards in enumerate(reward_list):
    plt.plot(rewards, label=f'Episode {i}')
plt.xlabel('Step')
plt.ylabel('Reward')
